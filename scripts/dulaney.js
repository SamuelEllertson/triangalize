...
class Delaunator {
    ...
    constructor(coords) {
        ...
        for (let i = 0; i < n; i++) {
            const d = dist_(cx, cy, coords[2 * i], coords[2 * i + 1]);
            if (d < minDist) {
                i0 = i;
                minDist = d;
            }
        ...
        for (let i = 0; i < n; i++) {
            if (i === i0) continue;
            const d = dist_(i0x, i0y, coords[2 * i], coords[2 * i + 1]);
            if (d < minDist && d > 0) {
                i1 = i;
                minDist = d;
            }
        ...
        for (let i = 0; i < n; i++) {
            dists[i] = dist_(coords[2 * i], coords[2 * i + 1], center.x, center.y);
        }
